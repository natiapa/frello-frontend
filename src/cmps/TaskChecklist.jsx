...
export function TaskChecklist({ checklists, onUpdated = () => {}, task, group, board }) {
  ...
  async function onRemoveChecklist(ev, checklistId) {
    ...
    const checklistsToSave = updatedCl.filter(checklist => checklist.id !== checklistId)
  ...
  async function handleChangeCheckbox({ target }, item, checklistId) {
    ...
    const checklistToUpdate = updatedCl.find(checklist => checklist.id === checklistId)
        ...
        {updatedCl &&
          updatedCl.map(checklist => (
            <li className='checklist' key={checklist.id}>
              ...
              {!isAddingItem && (
                <button className='add-item' onClick={ev => onAddItem(ev, checklist)}>
                  Add an item
                </button>
              )}
              {isAddingItem && checklist.isAddingItem && (
                <div className='add-an-item-form'>
                  ...
                  <div className='btns'>
                    ...
                    <button onClick={ev => oncancel(ev, checklist)}>Cancel</button>
                  </div>
                </div>
              )}
              <ChecklistItems
